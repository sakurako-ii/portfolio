...
<div class="right">
  ...
  <div class="text">
    ...
    ”個人レッスンPOP”などPOPの目的が分かるファイル名での<br>
    提出にご協力ください。確認後、速やかに掲載いたします。
    </p>
...
popularBtn.addEventListener('click', async () => {
  ...
  // ② IDをまとめる
  const ids = allImages.map(img => img.id).join(',');
...
//console.log('clicked:', item.label, item);
    ...
    if (item.label.includes('NEW')) {
  title.innerHTML = `
    ${item.label}
    <span class="new-desc">NEWに掲載中のコンテンツは、各カテゴリーメニューからもご覧いただけます。</span>
  `;
} else {
  title.textContent = item.label;
}
